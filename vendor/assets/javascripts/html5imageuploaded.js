...
(function (window, $, undefined) {
    ...
    $.html5imageupload = function html5imageupload(options, element) {
        ...
        this.button.rotate      = '<div class="btn btn-default btn-rorate title="' + (this.options.rotateTitle || 'Rotate') + '"><i class="glyphicon glyphicon-share-alt"></i></div>';
    ...
    $.html5imageupload.prototype = {
        _init: function() {
            ...
            if (!empty(options.image) && options.editstart == false) {
                ...
                //KNOWN BUG: fail to get mimetype, all images will be treated as PNG
                //however, front-end also wants to use PNG for resize purpose, so don't fix this
            ...
            // do background convertion for images > 512kb
            //if (Utils.dataURIToBlob(_self.options.image).size > 512000) {
            //  $(element).append('<div class="image-warning">Your image will be processed in the background.</div>');
            //}
        },
        _bind: function() {
            var _self			= this;
        ...
        readImage: function(image, src, name, mimeType) {
            ...
            img.onload 		= function(tmp) {
                ...
                _self.resizeToFillContainer();
            };
            img.src					= image;
        },

        resizeToFillContainer: function() {
            var _self = this;
                ...
                imageObj.onload = function() {
                    ...
                    tmpObj.onload = function() {
                        ...
                        if (options.save == false) {
                            ...
                            obj.rotateCount = _self.rotateCount;//shopmatic customization
        ...
        imageRotate: function() {
            var _self = this,
                wrapped_image = _self.image,
                image = wrapped_image[0],
                ghost = _self.imageGhost,
                canvas = document.createElement('canvas'),
                oldUseWidth = wrapped_image.data('useWidth'),
                oldWidth = wrapped_image.data('width');

            // shopmatic customization
            var rotateButton = $('.btn-rorate'),
                rotateButtonIcon = $('.btn-rorate > i');
            rotateButton.attr('disabled', true);
            rotateButtonIcon.removeClass('glyphicon-share-alt').addClass('glyphicon-time');
            image.onload = function() {
              _self.resizeToFillContainer();
              rotateButton.attr('disabled', false);
              rotateButtonIcon.removeClass('glyphicon-time').addClass('glyphicon-share-alt');
            };
            ...
            canvas.toBlob(function(blob) {
              var rotatedUrl = URL.createObjectURL(blob);
              image.src = rotatedUrl;
              $(ghost).attr('src', rotatedUrl);
              wrapped_image.data({
                useWidth: wrapped_image.data('useHeight'),
                useHeight: oldUseWidth,
                ratio: 1 / wrapped_image.data('ratio'),
                width: wrapped_image.data('height'),
                height: oldWidth
              });

              //shopmatic customization
              if (_self.rotateCount < 3) {
                _self.rotateCount++;
              } else {
                _self.rotateCount = 0;
              }
            }, wrapped_image.data('mime'));
        },
        imageFinal: function() {
            var _self			= this;
            var element			= _self.element;
        ...
        _tools: function() {
            ...
            var $image          = $(_self.image)
            var options			= _self.options;
            var zoomInOutFactor = Math.pow(2, $image.get(0).naturalWidth / $image.data('useWidth'));

            // rotation button
            $(tools).append($(_self.button.rotate).on({
                ...
                $(tools).append($(_self.button.zoomin).on({
                    'touchstart mousedown': function(e) {
                        ...
                        _self.interval 		= window.setInterval(function(){
                            _self.imageZoom(zoomInOutFactor);
                        },1);
                    },
